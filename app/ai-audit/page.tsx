...
export default function AIAuditPage() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // After 5 user messages, transition to lead capture
      if (count >= 5) {
        setTimeout(() => setPhase('capture'), 800);
      }
    } catch {
      ...
      <main className="flex-1 flex flex-col max-w-3xl w-full mx-auto px-4 py-6">
        ...
        {phase === 'intro' && (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
            ...
            <div className="relative mb-6">
              <div className="w-20 h-20 bg-gradient-to-br from-primary-500 to-purple-600 rounded-2xl flex items-center justify-center text-4xl shadow-lg">
                🧠
              </div>
            ...
            <p className="text-gray-500 mb-2 max-w-md">
              Riverr360's AI marketing consultant. Tell me about your business and I'll identify where your revenue might be leaking.
            </p>
        ...
        {(phase === 'chat' || phase === 'capture' || phase === 'done') && (
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1" style={{ maxHeight: 'calc(100vh - 340px)', minHeight: 300 }}>
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  {/* Avatar */}
                  {msg.role === 'assistant' && (
                    <div className="w-8 h-8 bg-gradient-to-br from-primary-500 to-purple-600 rounded-lg flex items-center justify-center text-sm flex-shrink-0 mt-1">
                      🧠
                    </div>
                  )}
                  {msg.role === 'user' && (
                    <div className="w-8 h-8 bg-gray-200 rounded-lg flex items-center justify-center text-sm flex-shrink-0 mt-1">
                      👤
                    </div>
                  )}
                  {/* Bubble */}
              ...
              {loading && (
                <div className="flex gap-3">
                  <div className="w-8 h-8 bg-gradient-to-br from-primary-500 to-purple-600 rounded-lg flex items-center justify-center text-sm flex-shrink-0">
                    🧠
                  </div>
            ...
            {/* Lead capture — shown after 5 messages */}
            {phase === 'capture' && !submitted && (
              <div className="bg-primary-50 border border-primary-100 rounded-2xl p-5 mb-4 flex-shrink-0">
                <p className="text-sm font-semibold text-gray-900 mb-1">
                  Based on our conversation, I've identified some key insights for you.
                </p>
                <p className="text-xs text-gray-500 mb-4">
                  Share your details to save this conversation and get your personalised follow-up.
                </p>
            ...
            {phase === 'done' && (
              <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-4 flex-shrink-0 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <Link href="/get-started" className="flex items-start gap-3 bg-primary-50 border border-primary-100 rounded-xl p-4 hover:bg-primary-100 transition-colors">
            ...
            {phase === 'chat' && (
              <div className="flex gap-3 flex-shrink-0">
                ...
                <button
                  ...
                  style={{ border: 'none', cursor: loading || !input.trim() ? 'default' : 'pointer' }}
            ...
            <p className="text-xs text-gray-400 text-center mt-3">
              Powered by Riverr360 · R360 Revenue Leakage Framework
            </p>
          </div>
        )}
