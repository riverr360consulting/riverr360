  ...
  return (
    <main>
          ...
          <div className="container-custom">
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-12">
              {featuredPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group bg-white border-2 border-primary-200 rounded-lg overflow-hidden hover:shadow-xl transition-shadow">
                  {post.coverImage && (
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-primary-600 font-semibold group-hover:translate-x-2 transition-transform">Read More →</span>
                    </div>
                  </div>
                </Link>
              ))}
      ...
      <section className="section-padding bg-gray-50">
        <div className="container-custom">
          <h2 className="text-3xl font-bold text-gray-900 mb-8">{featuredPosts.length > 0 ? 'Latest Posts' : 'All Posts'}</h2>
          {allPosts.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-xl text-gray-600 mb-6">No blog posts yet. Check back soon for marketing insights!</p>
              <p className="text-gray-500">To add posts, copy the template from <code className="bg-gray-200 px-2 py-1 rounded">content/blog/TEMPLATE-blog-post.md</code></p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {regularPosts.map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group bg-white rounded-lg overflow-hidden hover:shadow-xl transition-shadow">
                  {post.coverImage && (
                    <div className="aspect-video bg-gray-200 overflow-hidden">
                      <img src={post.coverImage} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    </div>
                  ...
                  <div className="p-6">
                    ...
                    <span className="text-primary-600 font-semibold text-sm group-hover:translate-x-2 transition-transform inline-block">Read More →</span>
                  </div>
                </Link>
              ))}
            </div>
